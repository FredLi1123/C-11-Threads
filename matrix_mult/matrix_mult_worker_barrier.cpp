...
void worker(double* matrix, int m, int n, double* x, double* y, int start, int start_tranpose, int block_size, int block_size_tranpose, spin_barrier& bar)
{
    ...
    // force all threads to finish executing the multiplication Ax before working on A^TAx
    bar.wait();
...
int main(){
  ...
  //random normal number generator and the distribution it follows
  ...
  //compute y = Ax and y = A^TAx in one run
  int block_start = 0;
  ...
  double diff=0;
  // compare the results of serial operations and parallel operations
  ...
  std::cout << "difference in serial versus multithread A^TAx is " << diff << std::endl;
